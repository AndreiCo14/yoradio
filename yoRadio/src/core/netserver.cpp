  ...
  if(xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS)){
    ...
    switch (request.type) {
      ...
      case GETINDEX:      {
        ...
        case GETSCHEDULE:   sprintf (wsbuf, "{\"sch_on\":\"%s\",\"sch_off\":\"%s\"}", 
                                  config.store.sch_on, 
...
Serial.println("=Write setting to EEPROM");
      ...
      if (strcmp(cmd, "sntp2") == 0) {
        config.saveValue(config.store.sntp2, val, 35, false); //no EEPROM.commit()
        return;
      }
      if (strcmp(cmd, "sntp1") == 0) {
//        strlcpy(config.store.sntp1, val, 35); //bug Аргументы всегда равны и не происходит запись
        bool tzdone = false;
